...
/* auto */ import { O, scontains, throwIfUndefined } from '../../ui512/utils/utilsAssert.js';
/* auto */ import { MapKeyToObject, Root, slength } from '../../ui512/utils/utilsUI512.js';
...
export class VpcAppPropPanel extends VpcAppInterfaceLayer {
    ...
    updateUI512Els(root: Root) {
        ...
        if (this.active) {
            this.active.refreshFromModel(root, this.appli.UI512App());
        }
    }

    saveChangesToModel(onlyCheckIfDirty: boolean) {
        if (this.active && getToolCategory(this.appli.getTool()) === VpcToolCtg.ctgEdit) {
            this.active.saveChangesToModel(this.appli.getRoot(), this.appli.UI512App(), onlyCheckIfDirty);
            this.updateUI512Els(this.appli.getRoot());
        }
    }
    ...
    areThereUnsavedChanges() {
        try {
            ...
            if (
                e.isUi512Error &&
                scontains(e.toString(), 'Simple Notification:') &&
    ...
    respondMouseDown(root: Root, d: MouseDownEventDetails) {
        if (this.active) {
            // any click, no matter where, saves changes
            this.active.saveChangesToModel(root, this.appli.UI512App(), false);
        }
    }
        ...
        if (this.active && d.elClick) {
            ...
            if (isOnPanel) {
                ...
                if (d.elClick.id && d.elClick.id.endsWith('##btnScript')) {
                    this.editor.respondToClick(root, this.appli.UI512App(), d.elClick.id);
