...
/* auto */ import { Root, cast, fitIntoInclusive } from '../../ui512/utils/utilsUI512.js';
...
export class VpcAppUIToolStamp extends VpcAppUIToolResponseBase {
    ...
    protected onChooseCategory(root: Root) {
        let ctg = this.getChosenCategory();
        if (ctg) {
            ...
            if (el) {
                ...
                if (el.get_ftxt().len() > 0) {
                    ...
                    SelAndEntry.selectLineInField(root, rghtgel, 0);
                    this.onChoosePicture(root);
                }
            }
        }
    ...
    protected onChoosePicture(root: Root) {
