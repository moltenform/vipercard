...
export class BuildFakeTokens {
    readonly canonicalImages: { [tknumber: number]: string } = {};
    syntaxMarker = tks.TokenTknewline;
    constructor() {
        this.canonicalImages[tokenType(tks.TokenTknewline)] = '\n';
        this.canonicalImages[tokenType(tks.TokenTkcomma)] = ',';
        this.canonicalImages[tokenType(tks.TokenTklparen)] = '(';
        this.canonicalImages[tokenType(tks.TokenTkrparen)] = ')';
        this.canonicalImages[tokenType(tks.TokenTo)] = 'to';
        Object.freeze(this.canonicalImages);
    }

    makeSyntaxMarker(basis: ChvIToken, whichMarker = '') {
        assertTrue(whichMarker === '' || whichMarker === ',', '8]|expected "" or ","');
        return whichMarker === ',' ? this.make(basis, tks.TokenTkcomma) : this.make(basis, this.syntaxMarker);
    }
    ...
    make(basis: ChvIToken, ctr: { new (...args: any[]): any }) {
        ...
        let image = this.canonicalImages[number];
