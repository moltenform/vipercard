...
t.test('testModelFindById.when exists and given correct type', () => {
        ...
        assertEq(VpcElType.Stack, this.vcstate.model.getById(VpcElStack, this.elIds.stack).getType(), 'HE|');
        assertEq(VpcElType.Card, this.vcstate.model.getCardById(this.elIds.card_a_a).getType(), 'HD|');
        assertEq(VpcElType.Btn, this.vcstate.model.getById(VpcElButton, this.elIds.btn_b_c_1).getType(), 'HC|');
});
t.test('testModelFindById.when exists and given incorrect type', () => {
        ...
        assertThrows('L/|', 'cast exception', () => this.vcstate.model.getCardById(this.elIds.stack));
        assertThrows('L.|', 'cast exception', () => this.vcstate.model.getById(VpcElButton, this.elIds.card_a_a));
        assertThrows('L-|', 'cast exception', () => this.vcstate.model.getById(VpcElStack, this.elIds.btn_b_c_1));
});
