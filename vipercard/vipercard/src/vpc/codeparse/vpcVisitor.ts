...
if (ctx.tkUnaryVipercardProperties && ctx.tkUnaryVipercardProperties[0]) {
...
if (ctx.tkStringLiteral && ctx.tkStringLiteral[0]) {
...
if (ctx.tkNumLiteral && ctx.tkNumLiteral[0]) {
 ...
 else { checkThrowInternal(false, '=C|OR in HBuiltinCmdPlay_1, no branch found'); }
}
