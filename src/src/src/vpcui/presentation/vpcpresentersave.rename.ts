...
/* auto */ import { BrowserOSInfo, Util512 } from '../../ui512/utils/utilsUI512.js';
...
export class VpcSaveUtils implements IVpcSaveUtils {
    busy = false;
    constructor(protected c: VpcPresenterInterface) {}

    mnuGoSave_As(): void {
        let ses = VpcSession.fromRoot(this.c.root);
        if (ses) {
            this.busy = true;
            this.goSave_AsAsync(ses);
        } else {
    ...
    mnuGoShareLink(): void {
        ...
        let br = this.c.root.getBrowserInfo();
    ...
    mnuGoExportJson(): void {
        ...
        this.goCountJsonSaves(currentusername, stackowner, stackid); // no need to synchronously block on it
        if (eThrown) {
            throw eThrown;
        }
    }
        ...
        if (this.c.cameFromDemoSoNeverPromptSave.length) {
            ...
            if (
                ...
                throw makeVpcInternalErr(
                    'Simple Notification: ' + this.c.lang.translate('lngFirst, go to File->Save to upload the stack.')
                );
            }
