...
/* auto */ import { O, cleanExceptionMsg, makeVpcInternalErr } from '../../ui512/utils/utilsAssert.js';
/* auto */ import { MapKeyToObjectCanSet, Root, slength } from '../../ui512/utils/utilsUI512.js';
...
export class VpcPanelScriptEditor extends UI512CompCodeEditor implements IsPropPanel {
    ...
    refreshFromModel(root: Root, app: UI512Application) {
        ...
        if (!vel) {
            ...
            this.refreshStatusLabels(root, app, vel);
        }
    }

    protected refreshStatusLabels(root: Root, app: UI512Application, vel: VpcElBase) {
    ...
    protected onBtnCompile(root: Root) {
        let vel = this.cbGetAndValidateSelectedVel('viewingScriptVelId');
        if (!vel) {
            return;
        }
        ...
        // run compilation
        this.saveChangesToModel(root, this.appli.UI512App(), false);
    ...
    scrollToErrorPosition(root: Root, c: O<UI512ControllerBase>) {
        ...
        if (this.el && linenotxt.length > 0) {
            ...
            if (Number.isFinite(thenum)) {
                ...
                SelAndEntry.selectLineInField(root, gel, thenum);

                if (c) {
                    c.setCurrentFocus(root, this.el.id);
                }
            }
        }
    }
    ...
    respondKeydown(root: Root, d: KeyDownEventDetails) {
        ...
        if (
            ...
            if (vel) {
                ...
                this.refreshStatusLabels(root, this.appli.UI512App(), vel);
            }
        }

        super.respondKeydown(root, d);
    ...
    respondToClick(root: Root, app: UI512Application, clicked: string): any {
        if (clicked.endsWith('##btnScript')) {
            ...
            if (short === 'caption') {
                ...
                this.saveChangesToModel(root, app, false);
                this.appli.setOption('viewingScriptVelId', '');
            } else if (short === 'btnScriptEditorCompile') {
                this.onBtnCompile(root);
            } else if (short === 'status2a') {
                this.scrollToErrorPosition(root, undefined);
    ...
    saveChangesToModel(root: Root, app: UI512Application, onlyCheckIfDirty: boolean) {
        ...
        if (onlyCheckIfDirty) {
            ...
            if (current !== newscript) {
                throw makeVpcInternalErr('Simple Notification: ' + VpcPanelScriptEditor.thereArePendingChanges);
            }
        } else {
