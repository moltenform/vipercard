...
export class EditTextBehavior {
    ...
    onMouseDownScroll(c: UI512PresenterWithMenuInterface, root: Root, d: MouseDownEventDetails) {
    ...
    onMouseDownSelect(c: UI512PresenterWithMenuInterface, root: Root, d: MouseDownEventDetails) {
        if (
            d.button === 0 &&
    ...
    onMouseMoveSelect(c: UI512PresenterWithMenuInterface, root: Root, d: MouseMoveEventDetails) {
    ...
    onMouseDoubleDown(c: UI512PresenterWithMenuInterface, root: Root, d: MouseDownDoubleEventDetails) {
        if (d.button === 0) {
            if (
                d.el &&
                d.el instanceof UI512ElTextField &&
    ...
    onMouseUp(c: UI512PresenterWithMenuInterface, root: Root, d: MouseUpEventDetails) {
        c.mouseDragStatus = MouseDragStatus.None;
    }

    onKeyDown(c: UI512PresenterWithMenuInterface, root: Root, d: KeyDownEventDetails) {
    ...
    onPasteText(c: UI512PresenterWithMenuInterface, root: Root, d: PasteTextEventDetails) {
        let el = SelAndEntry.getSelectedField(c);
    ...
    sendCutOrCopy(c: UI512PresenterWithMenuInterface, root: Root, el: UI512ElTextField, isCut: boolean) {
        if (el) {
            let gel = this.gelFromEl(el);
            if (!gel) {
                return;
    ...
    onIdle(c: UI512PresenterWithMenuInterface, root: Root, d: IdleEventDetails) {
