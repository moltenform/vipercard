...
/* auto */ import { RenderComplete, Root, assertEqWarn, fitIntoInclusive } from '../../ui512/utils/utilsUI512.js';
...
export class ScrollbarImpl {
    ...
    setScrollbarPositionsForRender(
        ...
        if (this.repositionScrollbarFieldShowScrollbar(el, pieces)) {
            ...
            this.setScrollbarPositionsForRenderGo(root, el, pieces, complete);
        } else {
            for (let key in pieces) {
                if (pieces.hasOwnProperty(key)) {
    ...
    setScrollbarPositionsForRenderGo(
        ...
        let scrollratio = this.repositionScrollbarGetThumbPos(root, el);
    ...
    getMouseCoordToSetCaret(root: Root, el: UI512ElTextField, x: number, y: number) {
        let [found, lowest] = this.getCoordToCharInField(root, el, x, y, false /* draw beyond visible */);
    ...
    getCoordToCharInField(
        ...
        let drawn = this.simulateDrawField(root, el, false /*measure height*/, drawBeyondVisible, cb);
        return drawn ? [found, lowest] : [undefined, undefined];
    }

    getCharacterInFieldToPosition(root: Root, el: UI512ElTextField, index: number) {
        ...
        let drawn = this.simulateDrawField(root, el, false /*measure height*/, true /*beyond visible*/, cb);
        return drawn ? found : undefined;
    }

    getScrollPosThatWouldMakeStartCaretVisible(root: Root, el: UI512ElTextField): O<number> {
        ...
        let contentHeightInPixels = this.getCachedHeightOfField(root, el);
        ...
        let found = this.getCharacterInFieldToPosition(root, el, index);
        if (found && found.length > 0) {
            let drawnY = found[1];
    ...
    getCachedHeightOfField(root: Root, el: UI512ElTextField) {
        let cachedHeight = el.get_n('contentHeightInPixels');
        if (cachedHeight && cachedHeight !== -1) {
            return cachedHeight;
        } else {
            let drawn = this.simulateDrawField(root, el, true /*measure height*/, true /*beyond visible*/, undefined);
            if (drawn) {
    ...
    repositionScrollbarGetThumbPos(root: Root, el: UI512ElTextField) {
        let contentHeightInPixels = this.getCachedHeightOfField(root, el);
    ...
    getApproxLineHeight(root: Root, el: UI512ElTextField, index: number) {
