...
export class VpcAppNonModalDialogSendReport extends VpcFormNonModalDialogFormBase {
    ...
    onClickBtn(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void {
        if (short === 'btnerrorlogs') {
            let lastClientLogs = UI512ErrorHandling.getLatestErrLogs(50).split('\n');
            ...
            this.doSendErrReport(root, this.appli);
        } else if (short === 'btnclose') {
            this.appli.setNonModalDialog(undefined);
        }
    }

    doSendErrReport(root: Root, appli: IVpcStateInterface) {
        let paramFields = this.readFields(appli.UI512App());
        let ses = VpcSession.fromRoot(root) as VpcSession;
        UI512BeginAsync(
            () => this.asyncSendErrReport(root, this.appli, paramFields['desc']),
    ...
    async asyncSendErrReport(root: Root, appli: IVpcStateInterface, userdesc: string) {
        let ses = VpcSession.fromRoot(root) as VpcSession;

        // get the last 30 logged errors, which might be useful.
        let lastClientLogs = vpcversion;
        lastClientLogs += '\n' + UI512ErrorHandling.getLatestErrLogs(30);
