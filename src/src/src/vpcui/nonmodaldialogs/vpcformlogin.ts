...
export class VpcFormNonModalDialogLogIn extends VpcFormNonModalDialogFormBase
    ...
    onClickBtn(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void {
        if (short === 'btnok') {
            if (this.waitingForVerifyCode) {
                ...
                this.doLoginVerifyCode(root, appli, this.waitingForVerifyCode);
            } else {
                this.setStatus(appli.lang(), 'lngLogging in...');
                this.doLogin(root, appli);
            }
    ...
    doLogin(root: Root, appli: IVpcStateInterface) {
    ...
    doLoginVerifyCode(root: Root, appli: IVpcStateInterface, keybuffer: ArrayBuffer) {
        let paramFields = this.readFields(appli.UI512App());
        UI512BeginAsync(
