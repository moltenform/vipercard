...
/* auto */ import { Root, Util512, assertEq, assertEqWarn } from '../../ui512/utils/utilsUI512.js';
...
export class Test_ScriptRun extends Tests_BaseClass {
    ...
    async initEnvironment(root: Root, callback: Function) {
        if (!this.initedAppl) {
            [this.ctrller, this.appl] = await this.startEnvironment(this.root)
            this.appl.appli.doWithoutAbilityToUndo( () =>
                this.populateModel())
            this.appl.appli.doWithoutAbilityToUndo( () =>
                this.ctrller.setTool(VpcTool.browse))
    ...
    async startEnvironment(root: Root) : Promise<[VpcAppController, VpcApplication]> {
        let lang = new UI512LangNull();
        let loader = new VpcDocLoader(lang, root, "", "", OpenFromLocation.NewDoc);
        await loader.loadDocumentTop(root)
    ...
    protected runGeneralCode(codeBefore:string, codeIn:string, expectErrMsg?: string, expectErrLine?: number, expectCompErr?:boolean, addNoHandler?:boolean) {
        let caughtErr = false;
        ...
        this.appl.runtime.codeExec.cbOnScriptError = (errFromScript) => {
            ...
            if (expectCompErr !== undefined && isCompilationStage !== expectCompErr) {
                ...
                if (!scontains(msg, expectErrMsg)) {
                    console.error("DIFFERENT ERR MSG for input " +
        ...
        let built = addNoHandler ? codeBefore + '\n' + codeIn :
            `${codeBefore}
            on mouseup
            ${codeIn}
            end mouseup`;
        ...
        this.appl.appli.doWithoutAbilityToUndo(() =>
            this.appl.runtime.codeExec.runTimeslice(Infinity))
...
put ${s} into testresult`;
    ...
    testBatchEvaluate(tests:[string, string][], floatingPoint=false) {
        ...
        for (let i=0; i<testsNoErr.length; i++) {
            ...
            if (floatingPoint) {
                ...
                if (this.evalHelpers.evalOp(VpcValN(parseFloat(expectString)), got,
                VpcOpCtg.OpEqualityGreaterLessOrContains, "==").readAsString() !== 'true') {
        ...
        for (let i=0; i<testsErr.length; i++) {
            ...
            let got = this.testOneEvaluate(beforeLine, expr, expectErr,
                errOnLine)
    ...
    tests = [
        ...
        (root: Root, callback: Function) => {
            this.initEnvironment(root, callback)
        },
        "test_jslru",
        () => {
...
put 1003 into x
end if` + '\\x && (counting() - cfirst)', '1000 2'],
                [`put counting() into cfirst
...
put 1003 into x
end if` + '\\x && (counting() - cfirst)', '1001 3'],
                [`put counting() into cfirst
...
put 1003 into x
end if` + '\\x && (counting() - cfirst)', '1002 4'],
                [`put counting() into cfirst
...
put 1003 into x
end if` + '\\x && (counting() - cfirst)', '1003 4'],
                // order matters
...
put s && "j" & j into s
put j+1 into j
end repeat\\s`, `a j0 j1 0 j0 j1 1 j0 j1 2 j2 j3`],
...
end repeat\\s`, `a 1,0 2,0 2,1 3,0 3,1 3,2`],
...
put s && i into s
put i+1 into i
end repeat\\s && (counting() - firstc)`, `b 0 1 2 5`],
...
put s && "a" into s
end repeat\\s && (counting() - firstc)`, `a a a a 5`],
                // "times" syntax rewriting, loop never done
                [`put "a" into s
put counting() into firstc
repeat (counting() * 0 + 0) times
put s && "a" into s
end repeat\\s && (counting() - firstc)`, `a 2`],
                // "times" syntax rewriting, loop never done
                [`put "a" into s
put counting() into firstc
...
put s && "a" & x into s
end repeat\\s && (counting() - firstc)`, `a a1 a2 a3 5`],
...
put s && "a" & x into s
end repeat\\s && (counting() - firstc)`, `a a3 a2 a1 a0 6`],
...
put s && "a" into s
end repeat\\s && (counting() - firstc)`, `a a a a 2`],
...
put s && "k" & k into s
end repeat\\s`, `a j0 j1 0 j0 j1 1 j0 j1 2 k1 k2 k3 k4`],
...
end repeat\\s`, `agot1got2got3got4got5`],
...
put "_" into s
        ...
        ()=>{
            ...
            this.appl.runtime.codeExec.globals.set("testresult", VpcValS("(placeholder)"));
            this.runGeneralCode(`on myhandler
global x
put x + 1 into x
end myhandler`, `global x, testresult
put 3 into x
myhandler
...
put 3 into x
myhandler2
put x into testresult`, 'no handler', 8)

            // longer call stack
            this.runGeneralCode(`on myhandler1
global x
...
myhandler3
put x && "h" into x
end myhandler4`, `global x, testresult
put "" into x
myhandler4
put x into testresult`)
            assertEqWarn(' myhandler4 myhandler3 myhandler2 myhandler1 j i h', this.appl.runtime.codeExec.globals.get("testresult").readAsString(), '22|')
...
on myhandler2
    myhandler1
end myhandler2`, `global testresult
myhandler2
put "a" & the result into testresult`)
            ...
            this.runGeneralCode(`on myhandler arg1
global x
...
end myhandlerMany`, `global x, testresult
put "a" into x
myhandler "hi"
myhandler "not"
...
myhandler "hi"
put x into testresult`)
            assertEqWarn('a myhandlerhi myhandlermanyh1h2h3 myhandlerhi', this.appl.runtime.codeExec.globals.get("testresult").readAsString(), '20|')
            // expect arguments eval'd from left to right
            this.runGeneralCode(`on myhandler a1, a2, a3
global x, testresult
...
myhandler counting(), counting(), counting()`)
            ...
            this.runGeneralCode(`on printargs a1, a2
global x
...
put " a1=" & a1 after x
put " a2=" & a2 after x
end printargs`, `global x, testresult
put "" into x
printargs
printargs "a"
printargs ("a")
...
#args=2 alla1=a alla2=b alla3= alla=a,b a1=a a2=b
#args=2 alla1=a alla2=b alla3= alla=a,b a1=a a2=b
#args=3 alla1=a alla2=b alla3=c alla=a,b,c a1=a a2=b`, this.appl.runtime.codeExec.globals.get("testresult").readAsString(), '1}|')
...
myhandler 14`)
            ...
            this.runGeneralCode(`on myhandler p
if p is 1 then
    return p
else
    myhandler p-1
...
put the result into testresult`)
            ...
            this.runGeneralCode(`${this.customFunc} myfn p
return p * (p + 1)
end myfn
`, `global testresult
...
this.runGeneralCode(`${this.customFunc} recurse p
global g
put g + 1 into g
if p is 1 then
    return p
...
put testresult && g into testresult`)
            ...
            // mutual recursion
this.runGeneralCode(`${this.customFunc} is_even n
if n is 0 then
return true
else
return is_odd(n - 1)
...
put is_even(8) && is_even(9) && is_even(10) into testresult`)
        ...
        this.updateObjectScript(this.elIds.card_a_a, `${this.customFunc} mm p1, p2, p3
        global g
        put g+1 into g
        return "m" & g & "(" & p1 & "," & p2 & "," & p3 & ")"
        end mm`)
        batch = [
            ...
            ['abs(1) && mm(90+(1),90+(2),"" & mm(90+(3),90+(4),"" & mm(90+(5),90+(6)))) && abs(1)', '1 m12(91,92,m11(93,94,m10(95,96,))) 1'],
            ...
            get 2
            end if\\it`, 'ERR:6:support custom fn calls'],
            ...
            show cd btn "notfound"\\0`, 'ERR:6:could not find the specified'],
            [`put "abc" into x


            show cd btn "notfound"\\0`, 'ERR:7:could not find the specified'],
            ...
            show cd btn "notfound"\\0`, 'ERR:7:could not find the specified'],
            [`put {BSLASH}\n "abc" {BSLASH}\n into {BSLASH}\n x
            show cd btn "notfound"\\0`, 'ERR:8:could not find the specified'],
            // but using put-expansion, line number reporting should not be affected, even though we're adding calls
            [`put mm(1) into x
            show cd btn "notfound"\\0`, 'ERR:5:could not find the specified'],
        ...
        // but not printargs("a")
        this.assertCompileErrorIn(`put 1 into x
sin(3)
put 1 into x`, `this isn't C`, 4)
        this.assertCompileErrorIn(`put 1 into x
myfn(3)
put 1 into x`, `this isn't C`, 4)

        // only block starts outside of scope
        this.assertCompileError(`put 1 into x
on myhandler
end myhandler`, 'can exist at this scope', 1)
        this.assertCompileError(`
on myhandler
end myhandler
put 1 into x`, 'can exist at this scope', 4)
        // cannot start handler inside handler
        this.assertCompileError(`
on myhandler
on myhandler2
end myhandler2
end myhandler`, 'inside an existing handler', 3)
        // cannot start handler inside handler
        this.assertCompileError(`
...
${this.customFunc} myhandler2
        ...
        this.assertCompileError(`
on myhandler1
end myhandler2`, 'names mismatch', 3)
        // doesn't make sense to end here
        this.assertCompileError(`
on myhandler1
end myhandler1
end myhandler1`, 'can exist at this scope', 4)
        // mismatched handler name in exit
        this.assertCompileError(`
on myhandler1
exit myhandler2
end myhandler1`, 'but got exit', 3)
...
end myhandler1`, 'names mismatch', 3)
        // no handler name
        this.assertCompileError(`
on
get 1 + 2
end myhandler1`, 'cannot have a line', 2)
        // no valid handler name
        this.assertCompileError(`
on ,
get 1 + 2
end myhandler1`, 'expected "on myhandler" but got', 2)
        // handler params invalid 1
        this.assertCompileError(`
on myhandler1 x y
get 1 + 2
end myhandler1`, 'required comma', 2)
        // handler params invalid 2
        this.assertCompileError(`
on myhandler1 x , ,
get 1 + 2
end myhandler1`, 'not a valid variable name', 2)
        // no handler end name
        this.assertCompileError(`
on myhandler1
get 1 + 2
end`, 'cannot have a line', 4)
...
next repeat`, 'outside of a loop', 5)
        ...
        end myhandler`, 'interleaved within', 5)
        ...
        this.assertCompileErrorIn(`if false then
        else then`, 'cannot have a line', 4)
        // cannot say just "if"
        this.assertCompileErrorIn(`if
        end if`, 'expected line to end with', 3)
        // cannot ommit the "then"
        this.assertCompileErrorIn(`if true
        end if`, 'expected line to end with', 3)
        ...
        this.assertCompileErrorIn(`repeat while
        end repeat`, 'without an expression', 3)
        // cannot just say "repeat until"
        this.assertCompileErrorIn(`repeat until
        end repeat`, 'without an expression', 3)
        // invalid repeat part 1
        this.assertLineError(`repeat xyz
        end repeat`, 'no variable found with this name', 3)
        // invalid repeat part 2
        this.assertLineError(`repeat to
        end repeat`, 'NoViableAltException', 3)
        // invalid repeat part 3, not quite enough tokens
        this.assertCompileErrorIn(`repeat with x = 1 to
        end repeat`, 'wrong length', 3)
        // invalid repeat part 4, not quite enough tokens
        this.assertCompileErrorIn(`repeat with x = 1 down to
        end repeat`, 'wrong length', 3)
...
end if`, 'interleaved within', 5)
        this.assertCompileErrorIn(`repeat while false
if false then
put "a" into x
else
end repeat
end if`, 'interleaved within', 7)
        this.assertCompileErrorIn(`if false then
repeat while false
end if
end repeat`, 'interleaved within', 5)
        this.assertCompileErrorIn(`if false then
repeat while false
else
put "a" into x
...
on myhandler
repeat while false
end myhandler`, 'interleaved within', 4)
        this.assertCompileError(`
on myhandler
if false then
end myhandler`, 'interleaved within', 4)
...
if false then
        ...
        this.runGeneralCode(`
on myhandler
put 3 into x`, '', 'at end of', 3, true, true)
        },
        ...
        ()=> {
            ...
            for (let parent of parents) {
                ...
                assertEqWarn(` me=${expectedMe} target=${this.elIds.btn_go}`, this.appl.runtime.codeExec.globals.get("testresult").readAsString(), '1[|')
                ...
                pass mouseUp
                put "b" after testresult`)
                assertEqWarn(`a me=${expectedMe} target=${this.elIds.btn_go}`, this.appl.runtime.codeExec.globals.get("testresult").readAsString(), '1>|')

                // local variables should not bleed over into another scope (upwards)
                this.appl.runtime.codeExec.globals.set("testresult", VpcValS(""));
                this.updateObjectScript(parent, `${this.customFunc} parentfn p1
                return "got" && myLocal && x && the short id of the target
                end parentfn`)
                this.assertLineError(`global testresult
                put "a" into myLocal
                put parentfn("abc") into testresult`, "no variable found", 2)
                ...
                put myLocal after testresult`, "no variable found", 3)
                ...
                this.appl.runtime.codeExec.globals.set("testresult", VpcValS(""));
                this.updateObjectScript(parent, `on mouseup
                global testresult
                put childfn("abc") into testresult
                end mouseup`)
                this.updateObjectScript(this.elIds.btn_go, `${this.customFunc} childfn p1
                return "got" && p1 && the short id of me
                end childfn`)
