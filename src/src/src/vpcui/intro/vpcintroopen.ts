...
/* auto */ import { Root, slength } from '../../ui512/utils/utilsUI512.js';
...
export class IntroOpenPage extends IntroPageBase {
    ...
    createSpecific(app: UI512Application, lang: UI512Lang) {
        ...
        if (this.openType === OpenFromLocation.FromStaticDemo) {
            ...
            this.getListChoicesAsync(this.root, prompt);
        }

        this.drawCommonLast(app, grp, lang);
    }

    async getListChoicesAsync(root: Root, prompt: UI512Element) {
        let ses = VpcSession.fromRoot(root);
        if (!this.chooser) {
            return;
        }
        if (ses) {
            try {
                ...
                if (this.root && this.loadedFromOnline.length) {
                    SelAndEntry.selectLineInField(this.root, new UI512ElTextFieldAsGeneric(this.chooser), 0);
                }
    ...
    static respondBtnClick(c: VpcIntroPresenterInterface, root: Root, self: IntroOpenPage, el: UI512Element) {
        if (el.id.endsWith('choicebtn0')) {
            let chosenId = IntroOpenPage.getChosen(self);
            if (chosenId && slength(chosenId)) {
                // open the document
                let loader = new VpcDocLoader(c.lang, root, chosenId, c.lang.translate('lngstack'), self.openType);
    ...
    respondKeyDown(c: UI512Controller, root: Root, d: KeyDownEventDetails) {
        /*
        if (d.readableShortcut === 'Delete' || d.readableShortcut === 'Backspace') {
            c.getModal().standardAnswer(this.root, c, c.app, 'Confirm deletion?', (n)=>{
                if (n===0) {
