...
export class Test_Parsing extends Tests_BaseClass {
    lexer: ChvLexer;
        ...
        if (this.parser.errors.length) {
            if (sErrExpected.length) {
                ...
                assertTrue(
                    ...
                    `1+|for input ${sInput} got different failure message, expected ${sErrExpected} ${this.parser.errors}`
                );
                return;
            } else {
    ...
    FnCall(
        FnCallNumberOf(
            ...
            this.assertFailsParseExp(`the number of words of bg 1`, `NoViableAltException: Expecting: one of these poss`);
...
AnyPropertyVal(
    ExprSource( 1 ) ) $set`
            ...
            this.assertFailsCmdSet(`set prop of cd btn 1 to 2 > 3`, `NotAllInputParsedException: Redundant input, expec`); // we only allow arith, nothing below
            this.assertFailsCmdSet(`set prop of cd btn 1 to 2 and 3`, `NotAllInputParsedException: Redundant input, expec`); // we only allow arith, nothing below
...
ObjectFld(
    ExprSource( 4 ) cd fld ) $set`
            ...
            this.assertFailsCmdSet(`set prop of word 1 to 2 of cd 3 to 4`, `NoViableAltException: Expecting: one of these poss`);
            this.assertFailsCmdSet(`set prop of word 1 to 2 of bg 3 to 4`, `NoViableAltException: Expecting: one of these poss`);
            this.assertFailsCmdSet(`set prop of word 1 to 2 of cd btn 3 to 4`, `NoViableAltException: Expecting: one of these poss`);
            this.assertFailsCmdSet(`set prop of word 1 to 2 of this stack to 4`, `NoViableAltException: Expecting: one of these poss`);
            this.assertFailsCmdSet(`set prop of word 1 to 2 of x to 4`, `NoViableAltException: Expecting: one of these poss`);
...
AnyPropertyVal(
    ...
    () => {
        // add
        this.testCmd('add x to y', `HContainer(
    HSimpleContainer( $y ) )
ExprSource(
    HSimpleContainer( $x ) ) $add to )`)
        this.testCmd('add 1+2+3 to y', `HContainer(
    HSimpleContainer( $y ) )
Lvl4Expr(
    ExprSource( 1 )
    ExprSource( 2 )
    ...
    OpPlusMinus( + )
    OpPlusMinus( + ) ) $add to )`)
        this.testCmd('add 1 to cd fld o', `HContainer(
    HSimpleContainer(
        ObjectPart(
            ObjectFld(
                ExprSource(
...
ExprSource( 1 ) $add to )`)
        this.testCmd('add 1 to line 2 of cd fld o', `HContainer(
    HChunk(
        HChunk_1(
            HChunkAmt( 2 ) )
        Of( of ) line )
        ...
        ObjectPart(
            ObjectFld(
                ExprSource(
                    HSimpleContainer( $o ) ) cd fld ) ) ) )
ExprSource( 1 ) $add to )`)
        this.testCmd('add 1 to line 2 to 3 of cd fld o', `HContainer(
    HChunk(
        HChunk_1(
            HChunkAmt( 2 )
            HChunkAmt( 3 ) to )
        ...
        ObjectPart(
            ObjectFld(
                ExprSource(
                    HSimpleContainer( $o ) ) cd fld ) ) ) )
ExprSource( 1 ) $add to )`)
        this.testCmd('add 1 to line 2 to (3*4) of cd fld o', `HContainer(
    HChunk(
        HChunk_1(
            HChunkAmt( 2 )
            HChunkAmt(
...
ExprSource( 1 ) $add to )`)
        ...
        // answer
        this.testCmd('answer x', `ExprSource(
    HSimpleContainer( $x ) ) $answer )`)
        this.testCmd('answer x \n y', `ExprSource(
    HSimpleContainer( $x ) )
ExprSource(
...
ExprSource(
    HSimpleContainer( $y ) ) $ask`)
        ...
        this.testCmd('go first cd', `NtDest(
    ObjectCard(
        HOrdinal( first ) cd ) ) $go )`)
        this.testCmd('go first bg', `NtDest(
    ObjectBg(
        HOrdinal( first ) bg ) ) $go )`)
        this.testCmd('go cd 2', `NtDest(
    ObjectCard(
        ExprSource( 2 ) cd ) ) $go )`)
...
ExprSource( "2" ) $multiply`)
        ...
        // drag
        this.testCmd('drag from 2,3 to 4,5', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 ) , , $drag $from to )`)
        this.testCmd('drag from 2,3 to 4,5 \n shiftkey', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 ) , , $drag $from $shiftkey
)`)
        this.testCmd('drag from 2,3 to 4,5 \n shiftkey, optkey', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 ) , , , $drag $from $shiftkey $optkey
)`)
        this.testCmd('drag from 2,3 to 4,5 \n shiftkey, optkey, cmdkey', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 ) , , , , $drag $from $shiftkey $optkey $cmdkey
)`)
        this.testCmd('drag from 2,3 to 4,5 to 6,7', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 )
ExprSource( 6 )
ExprSource( 7 ) , , , $drag $from to to )`)
        this.testCmd('drag from 2,3 to 4,5 to 6,7 \n shiftkey', `ExprSource( 2 )
ExprSource( 3 )
ExprSource( 4 )
ExprSource( 5 )
ExprSource( 6 )
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put "abc" \n into \n x', `ExprSource( "abc" )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put cd fld 1 \n into \n x', `ExprSource(
    HSimpleContainer(
        ObjectPart(
            ObjectFld(
                ExprSource( 1 ) cd fld ) ) ) )
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put sin(1) \n into \n x', `ExprSource(
    FnCall(
        FnCallWithParens(
            ExprSource( 1 ) $sin "(" ")" ) ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put result() \n into \n x', `ExprSource(
    FnCall(
        FnCallWithParens( $result "(" ")" ) ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the result \n into \n x', `ExprSource(
    FnCall(
        FnCallWithoutParensOrGlobalGetPropOrTarget( the $result ) ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the left of cd btn 1 \n into \n x', `ExprSource(
    ExprGetProperty(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the textfont of line 1 of cd fld 1 \n into \n x', `ExprSource(
    ExprGetProperty(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the long name of cd fld 1 \n into \n x', `ExprSource(
    ExprGetProperty(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the long version \n into \n x', `ExprSource(
    FnCall(
        FnCallWithoutParensOrGlobalGetPropOrTarget( the long $version ) ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put the number of cd btns \n into \n x', `ExprSource(
    FnCall(
        FnCallNumberOf(
            FnCallNumberOf_2( btns cd ) number the of ) ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put there is a cd btn y \n into \n x', `ExprSource(
    FnCall(
        ExprThereIs(
            Object(
                ObjectBtn(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put true or false \n into \n x', `Expr(
    ExprSource(
        HSimpleContainer( $true ) )
    ExprSource(
        HSimpleContainer( $false ) )
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put 2 > 3 \n into \n x', `Lvl1Expr(
    ExprSource( 2 )
    ExprSource( 3 )
    OpEqualityGreaterLessOrContains( > ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put 2 is a number \n into \n x', `Lvl2Expr(
    Lvl2Sub(
        Lvl2TypeCheck( number $a ) )
    ExprSource( 2 ) is )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put y is within z \n into \n x', `Lvl2Expr(
    Lvl2Sub(
        Lvl2Within(
            ExprSource(
                HSimpleContainer( $z ) ) within ) )
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put 4/5 \n into \n x', `Lvl5Expr(
    ExprSource( 4 )
    ExprSource( 5 )
    OpMultDivideExpDivMod( / ) )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put 4 div 5 \n into \n x', `Lvl5Expr(
    ExprSource( 4 )
    ExprSource( 5 )
    OpMultDivideExpDivMod( div ) )
HContainer(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put not true \n into \n x', `Lvl6Expr(
    ExprSource(
        HSimpleContainer( $true ) ) not )
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put first line of y \n into \n x', `Lvl6Expr(
    ExprSource(
        HSimpleContainer( $y ) )
    HChunk(
        HOrdinal( first )
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put line 1 to 2 of y \n into \n x', `Lvl6Expr(
    ExprSource(
        HSimpleContainer( $y ) )
    HChunk(
        HChunk_1(
...
HContainer(
    HSimpleContainer( $x ) ) $put $into`)
        this.testCmd('put not char 1 of (char 2 of y) \n into \n x', `Lvl6Expr(
    Lvl6Expr(
        ExprSource(
            HSimpleContainer( $y ) )
        HChunk(
