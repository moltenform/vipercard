...
/* auto */ import { Root, cast, slength } from '../../ui512/utils/utilsUI512.js';
...
export class VpcAppControllerEvents {
    ...
    initEvents() {
        ...
        this.c.listeners[UI512EventType.KeyDown.valueOf()] = [
            ...
            (a1: VpcPresenterInterface, a2: Root, a3: KeyDownEventDetails) =>
                VpcAppControllerEvents.respondKeyDown(a1, a2, a3, this.editTextBehavior),
            VpcAppControllerEvents.cancelEvtIfNotBrowseTool,
        ];

        this.c.listeners[UI512EventType.PasteText.valueOf()] = [
            (a1: VpcPresenterInterface, a2: Root, a3: PasteTextEventDetails) => {
                this.c.appli.undoableAction(() => {
                    this.editTextBehavior.onPasteText(a1, a2, a3);
                });
            },
        ];
    ...
    static cancelEvtIfCodeRunning(c: VpcPresenterInterface, root: Root, d: EventDetails) {
    ...
    static cancelEvtIfNotBrowseTool(c: VpcPresenterInterface, root: Root, d: EventDetails) {
    ...
    static respondMouseDoubleDown(c: VpcPresenterInterface, root: Root, d: MouseDownDoubleEventDetails) {
    ...
    static respondMouseDown(c: VpcPresenterInterface, root: Root, d: MouseDownEventDetails) {
        c.appli.undoableAction(() => {
            if (d.button === 0) {
                let isUserElOrBg = !!d.el && !!c.lyrModelRender.isVelOrBg(d.el.id);
                c.getToolResponse(c.appli.getTool()).respondMouseDown(root, c.appli.getTool(), d, isUserElOrBg);
                c.lyrNonModalDlgHolder.respondMouseDown(root, d);

                // change focus on click, to make the property panel commit
                let wasFocused = c.getCurrentFocus();
                c.setCurrentFocus(c.root, undefined);
                c.setCurrentFocus(c.root, wasFocused);
            }
        });
    }

    static respondMouseUp(c: VpcPresenterInterface, root: Root, d: MouseUpEventDetails) {
        c.appli.undoableAction(() => {
            if (d.button === 0) {
                let isUserElOrBg = d.getAffectedElements().some(item => !!c.lyrModelRender.isVelOrBg(item.id));
                c.getToolResponse(c.appli.getTool()).respondMouseUp(root, c.appli.getTool(), d, isUserElOrBg);
                c.lyrNonModalDlgHolder.respondMouseUp(root, d);
                ...
                c.lyrCoverArea.respondMouseUp(root, d);
            }
        });
    }

    static respondMouseMove(c: VpcPresenterInterface, root: Root, d: MouseMoveEventDetails) {
        let isUserElOrBg = d.getAffectedElements().some(item => !!c.lyrModelRender.isVelOrBg(item.id));
        c.getToolResponse(c.appli.getTool()).respondMouseMove(root, c.appli.getTool(), d, isUserElOrBg);
    ...
    static respondKeyDown(c: VpcPresenterInterface, root: Root, d: KeyDownEventDetails, ed: EditTextBehaviorSendToVel) {
    ...
    static scheduleScriptEvent(c: VpcPresenterInterface, root: Root, d: EventDetails) {
        VpcAppControllerEvents.scheduleScriptMsg(c, c.appli, d);
    }

    static respondKeyUp(c: VpcPresenterInterface, root: Root, d: EventDetails) {
    ...
    static respondMenuItemClicked(c: VpcPresenterInterface, root: Root, d: MenuItemClickedDetails) {
        c.appli.performMenuAction(d.id);
    }

    static respondIdle(c: VpcPresenterInterface, root: Root, d: IdleEventDetails) {
