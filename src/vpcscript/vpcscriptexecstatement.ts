...
export class ExecuteStatements {
    ...
    protected findChildOther<T extends VpcIntermedValBase>(typeAsString: string, vals: IntermedMapOfIntermedVals, nm: string): O<T> {
        let attempt1 = vals.vals[nm];
        if (attempt1) {
    ...
    go_answer(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        throw makeVpcScriptErr("5I|the 'answer' command is not yet implemented -- coming soon!");
    }
    ...
    go_ask(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        ...
        let asyncOp = () => {
            if (this.cbAskMsg) {
                this.cbAskMsg("prompt", "deftext", (s, n) => {
                    this.asyncOps.markCompleted(asyncOpId, [s, n]);
                });
            } else {
                throw makeVpcInternalErr("cbAskMsg not added to class");
            }
        };

        let retrieved = new refparam<any>(["", 0]);
        this.asyncOps.go(asyncOpId, asyncOp, blocked, retrieved);
        if (retrieved.val) {
            let [typedText, btnPressed] = retrieved.val;
            if (btnPressed === 3) {
                if (this.cbStopCodeRunning) {
                    this.cbStopCodeRunning();
                } else {
                    throw makeVpcInternalErr("cbStopCodeRunning not added to class");
                }
            } else {
                let s = (typedText || "").toString();
                this.outside.SetSpecialVar("it", VpcValS(s));
            }
        }
    }
    ...
    go_put(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        let terms = this.getAllChildStrs(vals, "TokenTkidentifier", true);
        checkThrow(terms.length === 2, "7M|we don't support 'put \"abc\"' to use the message box (missing into, before, or after).");
        let prep = getStrToEnum<RequestedChunkTextPreposition>(RequestedChunkTextPreposition, "RequestedChunkTextPreposition", terms[1]);
        let val = throwIfUndefined(this.findChildVal(vals, "RuleExpr"), "54|");
        let contref = throwIfUndefined(this.findChildOther<RequestedContainerRef>("RequestedContainerRef", vals, "RuleHContainer"), "53|");
    ...
    go_show(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        ...
        if (strs.length > 1) {
            if (strs[1] === "menubar") {
                ...
                throw makeVpcScriptErr("4}|so far we only support show menubar or show cd btn 1 or show cd btn 1 at x,y");
            }
        } else if (rule1) {
    ...
    go_sort(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        ...
        let contref = throwIfUndefined(this.findChildOther<RequestedContainerRef>("RequestedContainerRef", vals, "RuleHContainer"), "4[|");
        let cont = this.outside.ResolveContainerWritable(contref);
        let itemdel = this.outside.GetItemDelim();
    ...
    go_wait(line: VpcCodeLine, vals: IntermedMapOfIntermedVals, blocked: refparam<number>) {
        let asyncOpId = "single_thread_asyncOpId";
        let asyncOp = () => {
            setTimeout(400, () => this.asyncOps.markCompleted(asyncOpId, true));
        };

        let retrieved = new refparam<any>(false);
        this.asyncOps.go(asyncOpId, asyncOp, blocked, retrieved);
    }
}

export class ScriptAsyncOperations {
    static countId = new CountNumericId(10000);
    ...
    go(asyncId: string, asyncOp: () => void, isblocked: refparam<number>, retrieved: refparam<any>) {
