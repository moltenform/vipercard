...
export class UI512DemoTextEdit extends UI512Controller {
    ...
    private static respondMouseUp(c: UI512DemoTextEdit, d: MouseUpEventDetails) {
...
export class TestDrawUI512TextEdit extends UI512TestBase {
    ...
    simulateClick(
        ...
        if (doubleclick) {
            c.rawEvent(new MouseDownEventDetails(0, x, y, 0, mods));
            c.rawEvent(new MouseDownEventDetails(1, x, y, 0, mods));
            c.rawEvent(new MouseDownDoubleEventDetails(2, x, y, 0, mods));
        } else {
            c.rawEvent(new MouseDownEventDetails(0, x, y, 0, mods));
        }
    }
    ...
    drawTestCaseScrolling(c: UI512TestTextEditController) {
        ...
        this.simulateClick(c, 539, 118, false, false);
        for (let i = 0; i < 16; i++) {
            this.simulateKey(c, 'Q', 'q', true);
            this.simulateKey(c, 'W', 'w', true);
            this.simulateKey(c, 'E', 'e', true);
        }
        ...
        this.simulateClick(c, 360, 296, false, false);
        this.simulateKey(c, 'End', '', false, true);
        // hit Ctrl+Backspace 120 times
        for (let i = 0; i < 120; i++) {
            this.simulateKey(c, 'Backspace', '', false, true);
        }

        // try scrolling up/down with the arrows
        // insert some text at the beginning of fld 3
        this.simulateClick(c, 340, 110, false, false);
        for (let i = 1; i <= 8; i++) {
            this.simulateKey(c, i.toString(), i.toString(), true);
            this.simulateKey(c, 'Enter', '', false);
        }
        // hit the scroll down arrow 5x
        for (let i = 0; i < 5; i++) {
            this.simulateClick(c, 390, 222, false, false);
        }
        // hit the scroll up arrow 2x
        for (let i = 0; i < 2; i++) {
            this.simulateClick(c, 390, 111, false, false);
        }

        // try scrolling up/down by clicking on the background
        // click the lower bg of fld 5, 5x
        for (let i = 0; i < 2; i++) {
            this.simulateClick(c, 180, 345, false, false);
        }
        // hit the upper bg 2x
        for (let i = 0; i < 2; i++) {
            this.simulateClick(c, 178, 266, false, false);
        }

        // clicking the arrows should have no effect if scrollbar is disabled
        // click on the up/down arrows of fld 1
        for (let i = 0; i < 2; i++) {
            this.simulateClick(c, 177, 110, false, false);
        }
        for (let i = 0; i < 5; i++) {
            this.simulateClick(c, 177, 224, false, false);
        }
    ...
    drawTestCaseSelection(c: UI512TestTextEditController) {
        ...
        this.simulateClick(c, 444, 288);
        for (let i = 0; i < 6; i++) {
            this.simulateKey(c, 'ArrowDown', '', true);
        }

        // click to set focus left of the 12345 in fld 'halign+scr'
        // hit shift-left 4x
        this.simulateClick(c, 500, 118);
        for (let i = 0; i < 4; i++) {
            this.simulateKey(c, 'ArrowLeft', '', true);
        }

        // click to set focus left of the text in fld 'rdonly'
        // hit shift-left 5x
        this.simulateClick(c, 759, 135);
        for (let i = 0; i < 5; i++) {
            this.simulateKey(c, 'ArrowLeft', '', true);
        }
        ...
        // zero chars
        this.simulateClick(c, 78, 63);
        this.simulateKey(c, 'A', 'a', false, true);
        this.simulateKey(c, 'Delete', '', false, false);

        // 1 char, caret after
        this.simulateClick(c, 143, 60);
        this.simulateKey(c, 'A', 'a', false, true);
        this.simulateKey(c, '1', '1', false, false);

        // 1 char, selected
        this.simulateClick(c, 218, 60);
        this.simulateKey(c, 'A', 'a', false, true);
        this.simulateKey(c, '1', '1', false, false);
        this.simulateKey(c, 'ArrowLeft', '', true, false);

        // 1 char, caret before
        this.simulateClick(c, 291, 60);
        this.simulateKey(c, 'A', 'a', false, true);
        this.simulateKey(c, '1', '1', false, false);
        ...
        this.simulateClick(c, 618, 218);
        this.simulateKey(c, 'Z', 'z', true);
        this.simulateKey(c, 'ArrowRight', '', true);

        // click to set focus to the right of the 345 in fld 'halign'
        // type a to insert an a
        this.simulateClick(c, 638, 121);
        this.simulateKey(c, 'A', 'a', true);
        this.simulateKey(c, 'ArrowRight', '', true, true);

        // test rendering with right font after all chars deleted
        // click, hit Backspace 13 times, hit Delete 4 times
        // type Y
        this.simulateClick(c, 697, 219);
        for (let i = 0; i < 13; i++) {
            this.simulateKey(c, 'Backspace', '', false);
        }
        for (let i = 0; i < 4; i++) {
            this.simulateKey(c, 'Delete', '', false);
        }
        this.simulateKey(c, 'Y', 'y', false);
        this.simulateKey(c, 'A', 'a', false);
        this.simulateKey(c, 'B', 'b', false);
        ...
        this.simulateClick(c, 847, 205);
        this.simulateKey(c, '', '&', false);
        this.simulateKey(c, 'ArrowDown', '', true);
    }
}
