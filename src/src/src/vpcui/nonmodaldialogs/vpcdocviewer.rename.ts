...
/* auto */ import { Root, Util512, cast } from '../../ui512/utils/utilsUI512.js';
...
export class VpcAppNonModalDialogDocs extends VpcFormNonModalDialogBase {
    ...
    initialPopulate(root: Root) {
        ...
        SelAndEntry.selectLineInField(root, lftgel, 0);
        this.onChooseCategory(root, top);
    }
    ...
    protected onChooseCategory(root: Root, top: UI512ElTextField) {
    ...
    protected referenceShowData(grp: UI512ElGroup, btm: UI512ElTextField, ctg: number, jsonData: any) {
        ...
        let ln = SelAndEntry.selectByLinesWhichLine(gel);
        let entryTitle = entryTitles[ln as any];
        if (entryTitle) {
    ...
    protected onChooseItem(root: Root, btm: UI512ElTextField) {
        let grp = this.appli.UI512App().getGroup(this.grpid);
        let topGeneric = grp.getEl(this.getElId('topChoice'));
        let top = cast(topGeneric, UI512ElTextField);
        let ctg = this.getChosenCategoryNumber(top);
    ...
    createSpecific(app: UI512Application, lang: UI512Lang) {
        ...
        this.initialPopulate(this.appli.getRoot());
    }
    ...
    protected clickedBtnStartVid(root: Root, rightBtn: UI512Element) {
        if (this.type === DialogDocsType.Screenshots) {
            let grp = this.appli.UI512App().getGroup(this.grpid);
            let btmGeneric = grp.getEl(this.getElId('btmChoice'));
    ...
    protected clickedRightBtn(root: Root, rightBtn: UI512Element) {
        if (this.type === DialogDocsType.Screenshots) {
            ...
            if (lnCurrent !== undefined && lnCurrent < lastLine) {
                lnCurrent += 1;
                SelAndEntry.selectLineInField(root, gel, lnCurrent);
                this.onChooseItem(root, btm);
    ...
    onClickBtn(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void {
        if (short === 'topChoice') {
            this.onChooseCategory(root, cast(el, UI512ElTextField));
        } else if (short === 'btmChoice') {
            this.onChooseItem(root, cast(el, UI512ElTextField));
        } else if (short === 'rghtBtn') {
            this.clickedRightBtn(root, el);
        } else if (short === 'btnStartVid') {
            this.clickedBtnStartVid(root, el);
        }
    }

    onMouseDown(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void {}
}
