 ...
 *
 * uses SelAndEntryImpl to respond to events that change selection and content
 */
export class SelAndEntry {
    ...
    protected static changeTextInField(
        ...
        let amt = SelAndEntry.readOnlyScrollbarImpl.getScrollPosThatWouldMakeStartCaretVisible(el.getReadOnlyUI512());
        el.setScrollAmt(amt);
    }
    ...
    static changeTextDeleteLine(el: GenericTextField) {
        SelAndEntry.changeTextInField(el, (t, nCaret, nEnd) => SelAndEntryImpl.changeTextDeleteLine(t, nCaret, nEnd));
    }
    ...
    static changeSelCurrentWord(el: GenericTextField) {
        SelAndEntry.changeSelInField(el, (t, nCaret, nEnd) => SelAndEntryImpl.changeSelCurrentWord(t, nCaret, nEnd));
    }
    ...
    static changeSelPageUpDown(el: GenericTextField, isUp: boolean, isExtend: boolean) {
        let [nCaret, nEnd] = el.getSel();
        let approxLineHeight = SelAndEntry.readOnlyScrollbarImpl.getApproxLineHeight(el.getReadOnlyUI512(), nCaret);
        if (approxLineHeight) {
    ...
    static changeSelArrowKeyUpDownVisual(gel: GenericTextField, isUp: boolean, isExtend: boolean) {
        SelAndEntry.changeSelInField(gel, (t, nCaret, nEnd) => {
            let bounds = SelAndEntry.readOnlyScrollbarImpl.getCharacterInFieldToCoords(
