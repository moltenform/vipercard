        ...
        if (this.current) {
            if (d.elClick) {
                ...
                if (short === 'caption') {
                    ...
                    this.current.onClickBtn(short, d.elClick, root, this.appli);
                }
            }
        }
    }
    ...
    respondMouseDown(root: Root, d: MouseDownEventDetails) {
        if (this.current) {
            if (d.el) {
                ...
                if (short) {
                    this.current.onMouseDown(short, d.el, root, this.appli);
                }
            }
        }
    }
...
export abstract class VpcFormNonModalDialogBase extends UI512CompBase {
    abstract onClickBtn(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void;
    abstract onMouseDown(short: string, el: UI512Element, root: Root, appli: IVpcStateInterface): void;
