...
import { VpcSuperRewrite } from './vpcRewritesGlobal';
...
namespace NoElseIfClauses {
    ...
    function transformTreeRecurse(node:IfConstruct, output: ChvITk[][]) {
        ...
        for (let i=0; i<numberOfEndIfsNeeded; i++) {
            output.push([VpcSuperRewrite.tokenFromEnglishTerm('end', node.clauses[0].condition[0]),
                VpcSuperRewrite.tokenFromEnglishTerm('if', node.clauses[0].condition[0])])
        }
    }

    function goNoElseIfClauses(lines: ChvITk[][]) {
